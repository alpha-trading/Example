...
class Visualizer:
    ...
    def __init__(self, vnet=False):
        ...
        self.axes = None # 차트의 배열
        self.title = ''  # 그림 제목(탐험률, 파라미터...)
    ...
    def prepare(self, chart_data, title):
        ...
        with lock:
            ...
            # open, high, low, close 순서로된 2차원 배열 - 인덱스까지 5열(날짜)
            # 내가 date 지웠으니 차트 에서 전체다 읽어오기
            ohlc = np.hstack((x.reshape(-1, 1), np.array(chart_data)[:, :-1]))
            # np.hstack - 두 배열 유지하면서 합치기
        ...
        with lock:
            ...
            for idx, outval in zip(x, _outvals):
                ...
                if np.isnan(outval.max()):
                    continue
                if outval.argmax() == Agent.BUY_ten_per or outval.argmax() == Agent.BUY_two_per:
                    color = 'r'  # 매수 빨간색
                if outval.argmax() == Agent.BUY_thr_per or outval.argmax() == Agent.BUY_fou_per:
                    color = 'r'
