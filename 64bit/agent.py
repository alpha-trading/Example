...
"""
학습행동결정
지연보상 - 매도기준으로 손익률, 즉시보상 - 거래할때마다 -부여
(초반에 할때마다 -이지만 e으로 탐험하면 올라갈여지있음)
"""

class Agent:
    '에이전트 상태가 구성하는 값 개수'
    STATE_DIM = 2  # 주식 보유 비율, 포트폴리오 가치 비율 - 두가지 상태
    ...
    ACTIONS = [BUY_ten_per, BUY_two_per, BUY_thr_per, BUY_fou_per, ACTION_SELL, ACTION_HOLD]
    ...
    def decide_trading_unit(self, action):
        ...
        if action == Agent.ACTIONS[0]:
            ...
            if self.environment.get_closed_price() > 0:
                trading_size = int((self.portfolio_value * 0.1) / self.environment.get_closed_price() + 1e-5)

        if action == Agent.ACTIONS[1]:
            # 포폴기준 20퍼에 해당하는 주식수 return
            if self.environment.get_closed_price() > 0:
                trading_size = int((self.portfolio_value * 0.2) / self.environment.get_closed_price() + 1e-5)

        if action == Agent.ACTIONS[2]:
            # 포폴기준 30퍼에 해당하는 주식수 return
            if self.environment.get_closed_price() > 0:
                trading_size = int((self.portfolio_value * 0.3) / self.environment.get_closed_price() + 1e-5)

        if action == Agent.ACTIONS[3]:
            # 포폴기준 40퍼에 해당하는 주식수 return
            if self.environment.get_closed_price() > 0:
                trading_size = int((self.portfolio_value * 0.4) / self.environment.get_closed_price() + 1e-5)

        """if action == Agent.ACTIONS[4]:
            # 포폴기준 50퍼에 해당하는 주식수 return
            if self.environment.get_closed_price() > 0:
                trading_size = int((self.portfolio_value * 0.5) / self.environment.get_closed_price() + 1e-5)"""
        ...
        if _action == Agent.ACTIONS[0]:
            ...
            if trading_unit == 0:
                ...
                if invest_amount > 0:
                    ...
                    self.num_buy_10 += 1  # 매수 횟수 증가
                    self.buy_penalty_reward -= plus_penalty
                    self.ratio_hold += 10
                    self.num_buy += 1 # 총매수 횟수

        # 20% 매수
        if _action == Agent.ACTIONS[1]:
            # 매수할 단위를 판단
            ...
            if trading_unit == 0:
                ...
                if invest_amount > 0:
                    self.balance -= invest_amount  # 보유 현금을 갱신
                    self.num_stocks += trading_unit  # 보유 주식 수를 갱신
                    self.num_buy_20 += 1  # 매수 횟수 증가
                    self.buy_penalty_reward -= plus_penalty
                    self.ratio_hold += 20
                    self.num_buy += 1  # 총매수 횟수

        # 30% 매수
        if _action == Agent.ACTIONS[2]:
            # 매수할 단위를 판단
            ...
            if trading_unit == 0:
                ...
                if invest_amount > 0:
                    self.balance -= invest_amount  # 보유 현금을 갱신
                    self.num_stocks += trading_unit  # 보유 주식 수를 갱신
                    self.num_buy_30 += 1  # 매수 횟수 증가
                    self.buy_penalty_reward -= plus_penalty
                    self.ratio_hold += 30
                    self.num_buy += 1  # 총매수 횟수

        # 40% 매수
        if _action == Agent.ACTIONS[3]:
            # 매수할 단위를 판단
            ...
            if trading_unit == 0:
                ...
                if invest_amount > 0:
                    self.balance -= invest_amount  # 보유 현금을 갱신
                    self.num_stocks += trading_unit  # 보유 주식 수를 갱신
                    self.num_buy_40 += 1  # 매수 횟수 증가
                    self.buy_penalty_reward -= plus_penalty
                    self.ratio_hold += 40
                    self.num_buy += 1  # 총매수 횟수

        # 매도
        if _action == Agent.ACTIONS[4]:
            # 전량 매도
            # 매도
