...
""" 
실행모듈로 처음에는 실시간 거래 안하고 분석후 다음날 거래 -> 데이터받으면 바로 분석후 결정(실시간)
"""
if __name__ == '__main__':
    parser = argparse.ArgumentParser() # 프로그램 인자 만드는함수
    parser.add_argument('--stock_code', nargs='+')
    parser.add_argument('--ver', choices=['v1', 'v2'], default='v1')
    parser.add_argument('--rl_method', choices=['dqn', 'pg', 'ac', 'a2c', 'a3c'], default='dqn')
    parser.add_argument('--net', choices=['dnn', 'lstm', 'cnn'], default='dnn')
    parser.add_argument('--num_steps', type=int, default=1) # lstm, cnn 에서 사용할 스텝수
    ...
    parser.add_argument('--learning', action='store_true') # 입력되면 True 아니면 False
    ...
    value_network_path = ''
    policy_network_path = ''
    if args.value_network_name is not None: # 이름존재 하면
    ...
    for stock_code in args.stock_code:
        ...
        chart_data, training_data = data_manager.load_data(
            os.path.join(settings.BASE_DIR, '{}.csv'.format(stock_code)), ver=args.ver)
        # 파일위치 읽어오는거 수정주의
        ...
        if args.rl_method != 'a3c':
            ...
            if learner is not None:
                ...
                if args.learning == 0:
                    learner.save_last_action(args.stock_code)
